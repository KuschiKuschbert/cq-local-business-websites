...
rows = read_csv(p("approval_decisions.csv"))
...
write_csv(p("approval_decisions.csv"), rows, FIELDS)

generic = read_csv(p("decision_log.csv"))
generic.append({
    "date": today(),
...
write_csv(p("decision_log.csv"), generic, ["date", "decision", "area", "approved_by", "evidence_path", "notes"])
