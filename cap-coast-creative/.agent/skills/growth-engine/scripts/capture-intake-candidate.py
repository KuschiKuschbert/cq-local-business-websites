...
import argparse
import re
from common import clean, locked_csv_update, p, today
...
locked_csv_update(p("prospect_intake.csv"), FIELDS, upsert_candidate)

def append_attempt(attempts):
    attempts.append({
        "date": today(),
...
locked_csv_update(p("research_attempts.csv"), ATTEMPT_FIELDS, append_attempt)
