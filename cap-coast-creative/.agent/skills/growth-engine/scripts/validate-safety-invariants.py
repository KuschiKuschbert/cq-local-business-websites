...
unsafe_suppressed_operator_queue = [
    row for row in operator_queue
    if clean(row.get("status")) == "allowed-now"
    and any(
        term in " ".join([
            row.get("action", ""),
            row.get("evidence", ""),
            row.get("safe_command_or_next_step", ""),
            row.get("blocked_until", ""),
            row.get("notes", ""),
        ]).casefold()
        for term in ["suppressed-repeat-search", "suppress-repeat-search", "deprioritized-after-failures", "avoid repeated weak search"]
    )
]
checks.append({
    "invariant": "Suppressed research cannot be allowed-now",
    "status": "pass" if not unsafe_suppressed_operator_queue else "fail",
    "evidence": f"{len(operator_queue)} operator queue rows / {len(research_suppression)} suppression rows",
    "required_action": "Regenerate research experiments, source pivots, and operator queue so repeated failed routes cool down before more research.",
    "notes": "Suppressed or deprioritized research routes are not allowed-now." if not unsafe_suppressed_operator_queue else "Suppressed research route was allowed-now.",
})
