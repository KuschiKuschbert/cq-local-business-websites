...
decisions = {row.get("business"): row for row in read_csv(p("approval_decision_summary.csv"))}
...
for business, item in intake.items():
    ...
    decision = decisions.get(business, {})
    if clean(decision.get("decision")) == "hold":
        status = "on-hold"
    elif business in approvals:
    ...
    if business in research:
        points -= 10
    if status == "on-hold":
        next_action = "Promotion is on hold; gather sharper evidence or a clearer offer before asking again."
        evidence_path = clean(decision.get("evidence_path"), ".agent/memory/working/approval_decision_summary.csv")
    elif business in approvals:
