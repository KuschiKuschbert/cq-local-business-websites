...
approval_summary = {
    ...
    for row in read_csv(p("approval_decision_summary.csv"))
}
...
if clean(research_permission.get("status")) == "allowed":
    experiment = next((row for row in research_experiments if clean(row.get("status")) == "ready-to-test"), {})
    if not experiment:
        experiment = next((row for row in research_experiments if clean(row.get("status")) not in {"suppressed-repeat-search", "deprioritized-after-failures"}), {})
    experiment_status = clean(experiment.get("status"), "no-ready-experiment")
    queue_status = "allowed-now" if experiment_status == "ready-to-test" else "planned"
    add(
        rows,
        ...
        "Research public sources only; capture only strong public evidence or log a failed attempt." if queue_status == "allowed-now" else "Wait for a non-suppressed experiment or use a genuinely new source family.",
        "-" if queue_status == "allowed-now" else "A ready-to-test experiment exists.",
        clean(research_permission.get("safety_gate")),
        ".agent/memory/working/research_experiments.csv",
        f"Experiment status: {experiment_status}. No contact, login, form submission, DM, call, or social interaction.",
    )

for index, pivot in enumerate(source_pivots[:3], start=1):
    pivot_status = clean(pivot.get("status"))
    queue_status = "allowed-now" if pivot_status == "ready-for-pivot-research" else "planned"
    add(
        rows,
        ...
        "-" if queue_status == "allowed-now" else "A non-suppressed source-family pivot is available.",
...
for packet in approval_packets:
    ...
    business_key = clean(packet.get("business")).casefold()
    approval_status = clean(
        approval_summary.get((business_key, "promotion"), {}).get("status"),
        "pending",
    )
    if approval_status != "pending":
        continue
    inbox = approval_inbox.get(business_key, {})
    add(
        rows,
