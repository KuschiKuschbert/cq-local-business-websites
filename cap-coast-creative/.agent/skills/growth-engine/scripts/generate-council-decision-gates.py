...
for action in read_csv(p("action_permissions.csv")):
    ...
    gate_status = "blocked-by-action-permission" if action_status.startswith("blocked") else "advisory-only-requires-explicit-approval"
    if not debate:
    ...
    if action_name == "Run safe prospect research" and action_status == "allowed":
        gate_status = "allowed-with-council-constraints"
    ...
    if action_name == "Promote candidate to prospect" and action_status == "approval-recorded":
        gate_status = "human-approved-local-action-recorded"
    rows.append({
        "date": today(),
        "action": action_name,
        "council": council,
